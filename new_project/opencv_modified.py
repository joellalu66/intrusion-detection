...
# clear screen of previous messages
os.system('cls')
...
press ctrl + c to save event and exit
...
except KeyboardInterrupt: # save videos on exit 
    video.release()
    video_writer.release()
    cv2.destroyAllWindows()
    event = IntrusionEvent(
        video=f"./media/videos/intrusion_{timestamp}.mp4",
        image=f"./media/images/face_detected_{timestamp}.jpg"
    )
    event.save()
